...
*/
	...
	/*
	@RequestMapping(value = "/events", method = RequestMethod.POST)
	public ResponseEntity<Event> createEvent(@RequestParam(value="event")String eventJson, @RequestParam(value = "file") MultipartFile file,
			HttpServletRequest request,UriComponentsBuilder ucBuilder) {
		System.out.println("Creating Event " + event.getEventName());
		JSONObject jsonObj = new JSONObject(eventJson);
		System.out.println("Inside File upload" + eventJson);
		ObjectMapper mapper = new ObjectMapper().registerModule(new JsonOrgModule());

		 event = mapper.convertValue(jsonObj, Event.class);
			if (eventDao.isEventExist(event)) {
				System.out.println("A Event with name " + event.getEventName() + " already exist");
